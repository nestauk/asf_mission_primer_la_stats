from getting import get_emissions, get_regions
...
def form_emissions_data():
    ...
    la_pop_area = (
        region_emissions
        [['la_name', 'la_code', 'year', 'population', 'area', 'region']]
        .groupby(['la_name', 'year'])
        .head(1)
    ...
    all_emissions_area = (
        la_pop_area
        .merge(domestic_emissions, on = ['la_name', 'year'])
    )
    
    # Add additional columns
    all_emissions_area['emissions_per_capita'] = all_emissions_area['total_emissions'] / (all_emissions_area['population'])
    all_emissions_area['population_density'] = 1000 * all_emissions_area['population'] / all_emissions_area['area']
